...
```bash
TAG=$(curl -fsSL https://api.github.com/repos/macokay/proxmox-hive/releases/latest | grep -o '"tag_name":"[^"]*"' | cut -d'"' -f4) && sed -i "s|image: .*proxmox-hive:.*|image: ghcr.io/macokay/proxmox-hive:${TAG}|" /opt/proxmox-hive/docker-compose.yml && docker compose -f /opt/proxmox-hive/docker-compose.yml pull && docker compose -f /opt/proxmox-hive/docker-compose.yml up -d
```
